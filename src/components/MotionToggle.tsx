...
export default function MotionToggle({ className = "" }: { className?: string }) {
  ...
  return (
    <div
      className={`fixed right-3 top-3 z-[110] sm:right-5 sm:top-5 flex items-center gap-1 rounded-full border border-white/15 bg-black/70 p-1 text-[0.6rem] uppercase tracking-[0.18em] text-white/60 backdrop-blur-md ${className}`}
      role="group"
