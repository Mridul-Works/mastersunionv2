  ...
  return (
    ...
    >
      ...
      <div className="mt-3 overflow-hidden lg:mt-1.5" aria-hidden>
        <div
          className={`h-[2px] w-full origin-center bg-gradient-to-r from-transparent to-transparent ${dark ? "via-white/30" : "via-black/15"}`}
        />
      </div>
            ...
            >
              {universities.map((name) => (
                <div key={name} className="flex items-center justify-center">
                  <span
                    className={`whitespace-nowrap text-center text-[clamp(0.78rem,2.1vw,0.95rem)] italic tracking-[0.01em] transition-colors ${dark ? "text-white/[0.14] hover:text-white" : "text-black/[0.14] hover:text-black"}`}
