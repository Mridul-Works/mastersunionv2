...
const VENTURE_EASE = [0.7, 0, 0.2, 1] as const;
const VENTURE_SLIDE_DURATION = 0.95;
const VENTURE_COOLDOWN = 900;

function VentureStaggered({
  children,
  delay = 0,
  dur,
}: {
  ...
  return (
    <motion.div
      ...
      exit={{ y: -14, opacity: 0, filter: "blur(6px)", transition: { duration: dur(0.45) } }}
      transition={{ duration: dur(0.85), ease: VENTURE_EASE, delay: dur(delay) }}
    >
      {children}
    </motion.div>
  );
...
function VentureSlideVisual({ slide, direction, dur }: { slide: VentureSlideData; direction: 1 | -1; dur: (s: number) => number }) {
  return (
    <motion.div
      custom={direction}
      ...
      exit={{
        ...
        transition: { duration: dur(VENTURE_SLIDE_DURATION * 0.85), ease: VENTURE_EASE },
      }}
      transition={{ duration: dur(VENTURE_SLIDE_DURATION), ease: VENTURE_EASE }}
      className="absolute inset-0"
    >
      <motion.div
        className="flex h-full w-full items-center justify-center"
        initial={{ scale: 1.12, x: direction * 36 }}
        animate={{ scale: 1, x: 0 }}
        exit={{ scale: 1.04, x: -direction * 36 }}
        transition={{ duration: dur(VENTURE_SLIDE_DURATION * 1.2), ease: VENTURE_EASE }}
        style={{
          backgroundImage:
      ...
      >
        <div className="flex flex-col items-center gap-3">
          <span
            className="font-serif-italic text-background/10"
            style={{ fontSize: "clamp(90px, 16vw, 220px)", lineHeight: 1 }}
            aria-hidden
          >
            {slide.n}
          </span>
          <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-background/30">{slide.tag} image</span>
        </div>
      </motion.div>
      <div
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(180deg, rgba(10,10,10,0.7) 0%, rgba(10,10,10,0) 28%, rgba(10,10,10,0) 68%, rgba(10,10,10,0.95) 100%)",
        }}
      />
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-foreground/70 via-foreground/50 to-foreground/98 md:hidden" />
    </motion.div>
  );
}
...
function VentureSlide({
  ...
  slide: VentureSlideData;
  direction: 1 | -1;
  dur: (s: number) => number;
}) {
  return (
    <motion.div
      className="absolute inset-0 md:grid md:grid-cols-[1fr_460px]"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      transition={{ duration: dur(0.6), ease: "easeOut" }}
    >
      <div className="absolute inset-0 h-full w-full overflow-hidden md:relative md:inset-auto">
        <VentureSlideVisual slide={slide} direction={direction} dur={dur} />
      </div>

      <div className="absolute inset-0 z-10 flex items-end px-5 pb-20 pt-16 md:relative md:inset-auto md:items-center md:px-12 md:pb-0 md:pt-0">
        <div className="w-full max-w-[420px] bg-gradient-to-t from-foreground/95 via-foreground/80 to-transparent pb-3 pt-10 md:bg-transparent md:p-0">
          <VentureStaggered delay={0.15} dur={dur} key={`meta-${slide.n}`}>
          ...
          <VentureStaggered delay={0.4} dur={dur} key={`body-${slide.n}`}>
            <p className="mt-3 line-clamp-3 text-[12.5px] leading-[1.6] text-background/85 md:mt-6 md:line-clamp-none md:text-[13.5px] md:text-background/70">
              {slide.body}
            </p>
          </VentureStaggered>

          <VentureStaggered delay={0.5} dur={dur} key={`stat-${slide.n}`}>
            <div className="mt-4 flex items-end gap-3 md:mt-7 md:gap-4">
              <div className="text-[24px] font-black leading-none md:text-[28px]">{slide.stat}</div>
              <div className="pb-1 font-mono text-[9px] uppercase tracking-[0.18em] text-background/55">{slide.label}</div>
            </div>
          </VentureStaggered>
        </div>
      </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <div className="pointer-events-none absolute inset-0 bg-foreground/55 md:bg-foreground/40" />
        <div
          className="pointer-events-none absolute inset-0 hidden md:block"
          style={{
            background:
              "linear-gradient(90deg, rgba(10,10,10,0.1) 0%, rgba(10,10,10,0.35) 45%, rgba(10,10,10,0.88) 100%)",
          }}
        />
        <div
          className="pointer-events-none absolute inset-0 hidden md:block"
          style={{
            background:
              "linear-gradient(180deg, rgba(10,10,10,0.55) 0%, rgba(10,10,10,0) 30%, rgba(10,10,10,0) 55%, rgba(10,10,10,0.92) 100%)",
          }}
        />
        <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-foreground/65 via-foreground/45 to-foreground/98 md:hidden" />
        <div className="pointer-events-none absolute inset-0 hidden items-center justify-end overflow-hidden pr-[4vw] md:flex">
          <span
            aria-hidden
            className="font-black leading-none tracking-tighter text-transparent"
            style={{
              fontSize: "clamp(160px, 26vw, 360px)",
              WebkitTextStroke: "2px rgba(241,239,231,0.35)",
      ...
      <div className="relative z-10 grid h-full grid-cols-1 md:grid-cols-[3fr_2fr]">
        ...
        <div className="relative z-10 flex flex-col justify-end px-5 pb-16 pt-12 sm:pb-20 sm:pt-16 md:justify-between md:px-12 md:py-14">
          <VentureStaggered delay={0.15} dur={dur} key={`intro-meta-${index}`}>
            <div className="mb-auto flex items-start justify-between md:mb-0">
              <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-background">{slide.n} ventures</p>
              <p className="text-right text-[10px] uppercase tracking-[0.2em] text-background/40">
          ...
          <div className="w-full max-w-[420px] bg-gradient-to-t from-foreground/95 via-foreground/80 to-transparent pb-2 pt-6 sm:pb-3 sm:pt-10 md:bg-transparent md:p-0">
            ...
            <VentureStaggered delay={0.4} dur={dur} key={`intro-body-${index}`}>
              <p className="mt-2 line-clamp-2 text-[12px] leading-[1.5] text-background/85 sm:mt-3 sm:line-clamp-3 sm:text-[12.5px] sm:leading-[1.6] md:mt-6 md:line-clamp-none md:text-[13.5px] md:leading-[1.7] md:text-background/70">
                {slide.body}
              </p>
            </VentureStaggered>

            <VentureStaggered delay={0.5} dur={dur} key={`intro-stat-${index}`}>
              <div className="mt-4 h-px w-16 bg-background/30 md:mt-7" />
              <div className="mt-4 flex items-end gap-3 md:mt-5 md:gap-4">
                <div className="text-[24px] font-black leading-none md:text-[28px]">{slide.stat}</div>
                <div className="pb-1 font-mono text-[9px] uppercase tracking-[0.18em] text-background/55">{slide.label}</div>
              </div>
            </VentureStaggered>
          ...
          <VentureStaggered delay={0.6} dur={dur} key={`intro-cta-${index}`}>
            <button
              type="button"
              onClick={onStart}
              className="group mt-5 inline-flex min-h-11 items-center gap-3 text-[11px] uppercase tracking-[0.2em] text-background md:mt-0 md:text-[12px]"
            >
              <span className="relative">
                Start exploring
                <span className="absolute -bottom-1 left-0 h-px w-full origin-left scale-x-100 bg-background transition-transform duration-500 group-hover:scale-x-0" />
              </span>
...
function VentureDeck({ ventures }: { ventures: typeof DROPSHIPPING_TOP }) {
  ...
  const dur = useCallback(
    (seconds: number) => (prefersReducedMotion ? 0 : seconds),
  ...
  const go = useCallback(
    (next: number, dir: 1 | -1) => {
      ...
      setIndex((current) => {
        ...
        window.setTimeout(() => (lockRef.current = false), VENTURE_COOLDOWN);
        setDirection(dir);
        return clamped;
      });
    },
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => (visible = entry.isIntersecting && entry.intersectionRatio > 0.4),
      { threshold: [0, 0.4, 1] },
    );
    observer.observe(el);
  ...
  const slides: VentureSlideData[] = [
    {
      intro: true,
      n: String(ventures.length).padStart(2, "0"),
      tag: "ventures",
      headline: "Then I put it in front of real people.",
      body: "Four student-run stores from the Dropshipping Challenge — real products, real customers, real revenue. Swipe sideways.",
      stat: String(ventures.length).padStart(2, "0"),
      label: "Student ventures",
    },
    ...ventures.map((venture, i) => ({
      n: String(i + 1).padStart(2, "0"),
      tag: "Venture",
      headline: venture.name,
      body: venture.body,
      stat: venture.revenue,
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute left-5 top-5 z-30 font-mono text-[11px] tracking-[0.15em] text-background/70 md:left-10 md:top-6 md:text-[12px] md:text-background/45">
        ...
        <span className="mx-1 text-background/30">/</span>
        <span className="text-background/30">{String(total).padStart(2, "0")}</span>
      </div>

      <div className="pointer-events-none absolute left-0 right-0 top-5 z-30 hidden text-center sm:block md:top-6">
        <p className="text-[10px] uppercase tracking-[0.25em] text-background/45">Selected student ventures</p>
      </div>
      ...
      <AnimatePresence mode="popLayout" custom={direction}>
        {index === 0 ? (
          ...
          <VentureSlide key={slide.n} slide={slide} direction={direction} dur={dur} />
        )}
      </AnimatePresence>
      ...
      <button
        ...
        className="pointer-events-auto absolute bottom-5 left-5 z-40 rounded-full border border-background/30 bg-black/45 p-2.5 text-background backdrop-blur-sm transition hover:border-background/60 hover:bg-black/70 disabled:pointer-events-none disabled:opacity-20 md:bottom-auto md:left-8 md:top-1/2 md:-translate-y-1/2 md:p-3"
      >
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden className="rotate-180">
      ...
      <button
        ...
        className="pointer-events-auto absolute bottom-5 right-5 z-40 rounded-full border border-background/30 bg-black/45 p-2.5 text-background backdrop-blur-sm transition hover:border-background/60 hover:bg-black/70 disabled:pointer-events-none disabled:opacity-20 md:bottom-auto md:right-8 md:top-1/2 md:-translate-y-1/2 md:p-3"
      >
      ...
      <div className="pointer-events-none absolute inset-x-16 bottom-8 z-30 flex flex-col items-center gap-4 px-2 md:inset-x-0 md:bottom-7 md:px-6">
        <div className="hidden items-center gap-3 text-[10px] uppercase tracking-[0.22em] text-background/55 md:flex">
          <span className="inline-block h-1.5 w-1.5 rounded-full bg-background" />
          <span>Drag</span>
          <span className="text-background/20">·</span>
          <span>Arrows</span>
        </div>
        <div className="flex items-center justify-center gap-1.5 md:flex-wrap md:gap-2">
          {slides.map((s, i) => (
            <button
              key={`${s.n}-${i}`}
              onClick={() => go(i, i > index ? 1 : -1)}
              aria-label={`Go to slide ${i + 1}`}
              className="pointer-events-auto h-[2px] w-3 cursor-pointer overflow-hidden bg-background/20 md:w-8"
            >
              <span
                className="block h-full origin-left bg-background transition-transform duration-[700ms] ease-out"
  ...
  return (
    <Section id="dropshipping" tone="light">
      ...
      <div className="mt-12 sm:mt-14">
        <VentureDeck ventures={DROPSHIPPING_TOP} />
      </div>
      {typeof document !== "undefined" &&
        createPortal(
          <AnimatePresence>
