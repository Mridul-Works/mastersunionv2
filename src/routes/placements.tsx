...
function CinematicHero() {
  ...
  useEffect(() => {
    ...
    const off = onScrollFrame((s) => {
      // Read the driver's scroll value (the smooth-scroll layer's own offset,
      // flushed inside the same tick that writes the document scroll). Sampling
      // window.scrollY here instead reads a value from a different loop, so the
      // pinned layers fight the sticky box by a pixel or two each frame — that
      // is the up/down stutter at the top of the page.
      const y = s.y;
    ...
    <div className="career-experience-area">
      ...
      <Band tone="paper">
        ...
        <div className="career-coach-shell mt-6 sm:mt-12">
          ...
          <div
            ref={coachPanelRef}
            className="career-coach-panel"
            key={coachTrack}
            role="region"
            aria-roledescription="carousel"
            aria-label="Career coaching track"
            tabIndex={0}
        ...
        <div className="page-x">
          <div className="career-closing-cta">
            ...
            <nav aria-label="Explore Masters' Union">
              <Link to="/life-at-mu">Student Life <ArrowUpRight /></Link>
              <Link to="/placements">Explore Careers <ArrowUpRight /></Link>
              <a href="mailto:admissions@mastersunion.org">Book a Visit <ArrowUpRight /></a>
              <Link to="/life-at-mu">Explore Events <ArrowUpRight /></Link>
            </nav>
          </div>
        </div>
      </section>
      ...
      <div className="page-x relative w-full" style={{ zIndex: 3 }}>
        ...
        <div
          ...
          style={{
            ...
            bottom: "-1rem",
            height: "65%",
            background: "linear-gradient(to top, rgba(0,0,0,0.72) 0%, rgba(0,0,0,0.42) 42%, rgba(0,0,0,0.12) 76%, transparent 100%)",
          }}
        />
      ...
      <div className="page-x py-16 md:py-24">
        ...
        <div className="placement-data-bento grid overflow-hidden border border-white/15 lg:grid-cols-12">
          <header className="flex min-h-[260px] flex-col justify-between border-b border-white/15 p-7 md:p-10 lg:col-span-7 lg:border-r lg:border-b-0">
            <div className="flex items-center justify-between gap-6 pb-3 md:pb-0">
              <span className="pb-2 text-[10px] font-medium uppercase tracking-[0.22em] text-white/50" style={{ fontFamily: MONO }}>
              ...
              <thead>
                <tr>
                  {tbmColumns.map((column, index) => (
                    <th key={column.key} scope="col" className={`placement-ledger-cell px-5 py-5 text-[9px] font-medium uppercase tracking-[0.18em] text-white/42 ${index === 0 ? "w-[12%]" : ""}`} style={{ fontFamily: MONO }}>
                      {column.label}
                    </th>
                  ))}
                </tr>
        ...
        <div className="mt-10 grid border border-white/15 bg-[#0A1712] lg:grid-cols-12">
          <div className="flex flex-col justify-between border-b border-white/15 p-7 pb-5 md:p-8 lg:col-span-3 lg:border-r lg:border-b-0">
            ...
            <div className="mt-5 md:mt-14 lg:mt-0">
              ...
              <thead>
                <tr>
                  {ylcColumns.map((column) => (
                    <th key={column.key} scope="col" className="placement-ledger-cell px-5 py-5 text-[9px] font-medium uppercase tracking-[0.18em] text-white/42" style={{ fontFamily: MONO }}>{column.label}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
            ...
            <div className="col-span-2 border-b border-white/10 pb-8 sm:col-span-3 md:col-span-5 md:border-b-0 md:pb-0">
              ...
              <div className="mt-5 flex items-center gap-2.5 sm:mt-6">
                {[Instagram, Linkedin, Youtube, Twitter].map((Icon, i) => (
                  <a key={i} href="#" aria-label={["Instagram", "LinkedIn", "YouTube", "X"][i]} className="flex size-10 shrink-0 items-center justify-center rounded-none border border-white/15 text-white/60 transition-colors hover:border-white hover:text-white">
